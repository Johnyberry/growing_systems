...
def brain_link(HexCenter, edges ):
    ...
    while len(pts) <= 10:
        x = 2*random.uniform(-3, 3.0)
        y = 2*random.uniform(-3, 3.0)
        z = random.uniform(0.0, 10)
        vect = (x, y, z)
        newPt = rs.CopyObject(pts[-1],vect)
        distance = rs.Distance(newPt, pts[-1])
        if distance > 6:
...
def Hex_grid( HexCenter, HexExtend, HexSize, HexAngle, edges, brep):
    ...
    for p in new_points:
        brain_link(p,(edges/16))
        #modular_unit(p,(edges/16),center=None)
...
def main():
    ...
    HexExtend = rs.GetInteger("Enter the number of radial levels", 2
    , 2)
